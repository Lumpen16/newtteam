...
import "./CallbackWrapper.css"

export default class CallbackWrapper extends React.Component<{}, {passed: boolean, width: number, height: number}> {
    ...
    handlePass () {
        this.setState({passed: true, width: (document.getElementById("test") as HTMLElement)?.offsetWidth, height: (document.getElementById("test")as HTMLElement)?.offsetHeight});
        setTimeout(() => {this.setState({passed: false})}, 5000)
    }
    handleClose () {
        this.setState({passed: false, width: 0, height: 0});
    }
    render () {
        return (
            <div id={"test"} className="callback_wrapper">
                ...
                <p style={{fontWeight: 500}}>Нажимая на кнопку, вы соглашаетесь с <span className="callback_wrapper_underlined">условиями сервиса</span></p>
            </div>
        )
    }
}
